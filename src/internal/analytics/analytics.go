...
func Get(parameter gin.Param) (Analytics, error) {
	mapFunction, parameterIsFound := analyticTypes[parameter.Key]
	...
	if parameterIsFound {
		...
		_, quarterValueIsValid := allowedQuarterFormats[paramAsANumber]

		if !quarterValueIsValid {
			err = quarterFormatError
			return Analytics{}, err
		}
